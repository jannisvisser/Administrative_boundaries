...
datasets = Dataset.search_in_hdx('',fq='tags:hxl,population')
dataset_id=1
...
#	if tag in dataset.get_tags():
	...
	print(country)
	if not ',' in country:
		country_dir = downloadpath + country.replace(':','') + '/'
...
#					pcode_col,name_col,pop_col,field_names=get_field_names(resource['shape_info'],admin_level)
			data = pd.DataFrame.from_records([{'dataset_id': dataset_id, 'resource_id': resource_id, 'country': country, 'name': resource['name'], 'adm': admin_level, 'link': resource['url']}])
			df = df.append(data)
			resource_id+=1
		dataset_id+=1

df.to_csv(downloadpath+'metadata_hdx_api_census.csv')	
